...
class PetersonGrahamScannerOMP : public ppc::task::Task<InputValue, OutputValue> {
 ...
 private:
  ...
  static void SortByAngleParallel(PointSet &points);

  // Вспомогательные методы для параллельной сортировки
  static void ParallelQuickSort(PointSet &points, int left, int right, const Point2D &origin);
  static int Partition(PointSet &points, int left, int right, const Point2D &origin);
};
